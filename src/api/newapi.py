...
import os
import pandas as pd
from src.constants import P4_MOUSE_REFERENCE_ID, P56_MOUSE_REFERENCE_ID, CHUNK_HEADERS, HEADERS_V4, CHUNK_HEADERS_V2, CHUNK_HEADERS_V4, P4_CONVERSION, P56_CONVERSION
...
class Api():
    ...
    def start_ref_to_img(self, voxels_path: str, section_ids_path: str, dir_path: str, headers: list[str], thread_chunk_size: tuple = None, file_name: str = "Chunk") -> None:
        '''
        ...
        if headers == CHUNK_HEADERS_V2:
            for row in df.itertuples():
                ...
                self.reference_to_image(row.X, row.Y, row.Z, headers, section_ids, new_path)

        elif headers == CHUNK_HEADERS_V4:
            df.rename(columns = {"Structure-ID":"structure"}, inplace=True)
            for row in df.itertuples():
                new_path = f"{full_path}_{row.Index}.csv"
                self.reference_to_image(row.X, row.Y, row.Z, headers, section_ids, new_path, row.structure)

    def start_img_to_ref(self, chunk_path: str, dir_path: str, headers: list[str], thread_chunk_size: tuple = None, file_name: str = "Voxel") -> None:
        '''
        ...
        if headers == HEADERS_V4:
            ...
            for row in df.itertuples():
                new_path = f"{full_path}_{row.Index}.csv"
                self.image_to_reference(row.Section_Image, row.Seed_x, row.Seed_y, new_path)
    ...
    new_path: str
        ...
        if not path_exists(new_path):
            with open(new_path, mode="w", newline="") as new_file:
                ...
                writer.writerow(headers)
                query = self.query.url
                query = query.format(section_image_id=section_image_id, x_coord=X, y_coord=Y)
                print(query)
                #response = requests.get(query)
