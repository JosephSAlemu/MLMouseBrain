...
def errored_queries() -> None:
    ...
    for url in urls:
        ...
        created_file = (
            rf"./Datasets/Outputs/Fill_Negatives/error/P4_Chunk_{counter}.csv"
        )
        with open(created_file, mode="a", newline="") as new_file:
            writer = csv.writer(new_file)
            writer.writerow(CHUNK_HEADERS_V2)
...
def image_to_reference(
    section_image_id: int, x_coord: int, y_coord: int
) -> tuple | None:
    """
