...
from src.utils.filter import (get_all_chunks, group_data, get_section_dataset_ids, read_chunk_files, create_sub_voxel_dataframe, bin_expression_values, missing_and_empty_distributions_voxels, missing_and_empty_distributions_genes)
from src.utils.validation import (is_valid_image)
...
def histogram_negative_distribution(type: Action, right_inclusive: bool, path: str) -> None:
    """
    generate a histogram of # of voxels with 0-10% 0/-1s, 10-20% 0/-1s, 20-30% 0/-1s
    """
    match type:
        case Action.VOXELS:
            bin_edges, values = missing_and_empty_distributions_voxels(path)
            type = "Voxels"

        case Action.GENES:
            bin_edges, values = missing_and_empty_distributions_genes(path)
            type = "Genes"


    missing_zero_values = pd.Series(values)
    binned = pd.cut(missing_zero_values, bins=bin_edges, right=right_inclusive)
    counts = binned.value_counts(sort=False)
    print(counts)
    print(counts.sum())
    plt.bar(counts.index.astype(str), counts.values)
    plt.xlabel("Bins")
    plt.ylabel("Frequency")
    plt.title("0 or -1 expressions for " + type)
    plt.show()

def get_valid_boxes(section_image_id: str, func: int) -> list[tuple,tuple]:
    '''
    Takes in a section_image_id
    retrieves the section_dataset_id associated with the section_image_id
